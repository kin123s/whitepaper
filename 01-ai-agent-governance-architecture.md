...
> **한계점:** 규칙(`rules.md`)에 "검색하라"고 적는 것만으로는 부족하다. 실측 데이터가 없는 규칙은 공허하다. 결국 거버넌스는 **검색할 수 있는 지식 인프라(RAG Server)**와 결합될 때 비로소 완성된다. (→ [Chapter 3. 지식 미들웨어 구축기](./04-mcp-rag-server-architecture.md) 참조)
...
└─────────┬────────────────────────────────────────────────────┘
      ...
      (Proposed)
          ▼
┌──────────────────────────────────────────────────────────────┐
│          Knowledge Layer (GraphRAG Backbone)                  │
│  "규칙이 참조할 실측 데이터 — 260K 노드 기반 지식 그래프"    │
└──────────────────────────────────────────────────────────────┘
```
...
## 7. MCP Integration (Phase 1)
#### *"검색하라고 명령했다. 하지만 검색할 곳이 없었다."*

초기 거버넌스 단계에서의 MCP(Model Context Protocol)는 단순히 **규칙의 보조 도구**였다. `mcp-usage.md` 규칙을 통해 "추측하지 말고 검색하라"고 에이전트에게 강제했지만, 당시에는 인덱싱된 데이터가 부족하여 검색 결과가 나오지 않는 경우가 많았다.

- **초기 시도:** `grep_search` 중심의 단순 키워드 검색.
- **문제점:** 레거시 코드의 복잡한 의존성(함수 호출 관계 등)을 키워드로만 찾는 데 한계.
- **결론:** 에이전트에게 "공부해라(규칙)"고 하는 것보다, "원하는 정보를 즉시 찾아주는 인프라(RAG)"가 필요함을 깨달음. (→ [Chapter 3. 지식 미들웨어 구축기](./04-mcp-rag-server-architecture.md)에서 해결)
...
#### Lesson 6 (Critical): "규칙은 Fact를 이길 수 없다"
가장 뼈아픈 교훈이다. "DB 스키마를 추측하지 마라"는 규칙을 100번 적어도, AI가 당장 코드를 짜야 하는데 볼 수 있는 스키마 문서가 없으면 결국 환각을 일으켰다. 거버넌스 체계(How)를 완성했으니, 이제 그 체계가 먹어 치울 **실측 데이터(What)**를 확보하는 인프라가 다음 과제가 되었다.

---

## 11. Next Journey: Episode 1 Conclusion

3-Layer 규칙 시스템을 통해 AI 에이전트의 거친 행동을 통제하고 전사적인 질서를 잡는 데 성공했다. 하지만 이 시스템은 하나의 거대한 **"그릇"**에 불과했다.

- **Problem:** 규칙은 완벽하지만, 참조할 지식이 머릿속(Training Data)에만 있는 AI.
- **Solution:** 사내의 모든 코드와 DB 정보를 실시간으로 검색할 수 있는 인프라가 필요하다.
- **Next Episode:** [Chapter 2. Legacy Modernization](./03-legacy-modernization.md)을 통해 코드 구조를 먼저 정리하고, [Chapter 3. MCP RAG Server](./04-mcp-rag-server-architecture.md)에서 본격적인 지식 엔진을 구축하기 시작한다.
