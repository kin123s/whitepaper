...
사내 레거시 PHP 코드베이스(20만 줄), 10년치 DB 스키마(DDL 7,000줄), 3개 프로젝트 — 이걸 토큰 안에 넣을 수는 없었다. 그렇다고 사내 코드를 외부 SaaS에 보낼 수도 없었다. **자체 구축** 외에 선택지가 없었다.
...
> **Note:** 아래 수치는 초기 RAG 서버(v1.0, ChromaDB 기반) 운영 시점의 측정값입니다.
> GraphRAG(v11.0) 진화 이후의 지표는 [Chapter 3-2 문서](./05-evolution-to-graphrag.md)를 참조하세요.

| 지표 | 값 | 비고 |
|------|---|------|
| 인덱싱된 문서 수 (v1.0) | ~2,500개 | Git 로그 + DDL + 프로젝트 문서 |
| 인덱싱된 전체 파일 수 (v11.0) | **34,678개** | GraphRAG full 인덱싱 완료 (2026-04-06) |
